...
A complete end-to-end, locally hosted voice interface demo combining browser-based audio capture, client-side Voice Activity Detection (VAD), low-latency streaming Speech-to-Text (ASR), Large Language Model (LLM) text transformation, and Text-to-Speech (TTS) voice feedback—running fully offline natively on Windows, Linux, and macOS.
...
5. **TTS Voice Feedback**: Synthesizes localized voice responses using **CrispASR (Piper Backend)**.
...
### 1. Executables
* **CrispASR**: Native streaming Speech-to-Text server and embedded Piper TTS engine (`bin/crispasr` or `bin/crispasr.exe`)
* **llama.cpp**: High-performance inference engine powering `llama-server` (`bin/llama-server` or `bin/llama-server.exe`)
...
* **Nemotron 3.5 Streaming ASR (GGUF)**: [nemotron-3.5-asr-streaming-0.6b-q4_k.gguf](https://huggingface.co/nvidia/nemotron-3.5-asr-streaming-0.6b)
* **Gemma 4 E2B (GGUF)**: [gemma-4-E2B-it-UD-Q4_K_XL.gguf](https://huggingface.co/google/gemma-4-E2B)
* **Piper TTS Voices (GGUF)**: [LocalAI-Community/piper-voices-GGUF](https://huggingface.co/LocalAI-Community/piper-voices-GGUF/tree/main)

---

## 🖥️ Server Setup Instructions

Open separate command prompt or terminal windows to launch each component, or use the **companion orchestrator** to launch all 3 components automatically (`companion/start_companion.bat` or `./companion/start_companion.sh`).

### Step 1: Start CrispASR TTS Server (Port 8089)
```bash
bin/crispasr --server --backend piper -m "models/hi_IN-rohan-medium.gguf" --port 8089 -l hi -t 8 --no-spoken-disclaimer --accept-marking-responsibility
```
...
```bash
CRISPASR_NEMOTRON_STREAMING=1 bin/crispasr --server --backend nemotron -m "models/nemotron-3.5-asr-streaming-0.6b-q4_k.gguf" -l hi --stream --stream-json --stream-final-mode prefix --port 8080 --ws-port 8081
```

### Step 3: Start Gemma 4 E2B LLM Server (Port 8084)
```bash
bin/llama-server --model "models/gemma-4-E2B-it-UD-Q4_K_XL.gguf" --port 8084 --reasoning off
```
...
* `silero_vad.onnx` - Silero Voice Activity Detection ONNX weights.
...
* **[Piper Voices (Rhasspy)](https://huggingface.co/rhasspy/piper-voices)**
